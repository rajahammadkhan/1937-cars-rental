<script type="text/javascript">
    ...
    var table = $('#datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('mf_business_setup.datatable') }}",
        "columns": [{
                "data": "id",
                "defaultContent": ""
            },
            ...
            @if(auth()->user()->can('mf_business_setup-edit') || auth()->user()->can('mf_business_setup-delete'))
            {
                "data": "id",
                "defaultContent": ""
            },
        ...
        "columnDefs": [{
            ...
            },
            {
                ...
                "render": function(data, type, row, meta) {
                    ...
                    var edit = "{{ route('mf_business_setup.edit', [':id']) }}";
                    edit = edit.replace(':id', data);

                    var header_content = "{{ route('mf_business_setup.header_content', [':id']) }}";
                    header_content = header_content.replace(':id', data);

                    return `<div class="text-center">
                          
                          @can('mf_business_setup-add header content')
        ...
        $('.status').change(function () {
          ...
          axios
            .post('{{route("mf_business_setup.status")}}', {
              _token: '{{csrf_token()}}',
              _method: 'patch',
              id: id,
              status: status,
        ...
        $('.delete').click(function () {
          ...
          swal({
            ...
            if (result.value) {
            axios
              .post('{{route("mf_business_setup.destroy")}}', {
                _method: 'delete',
                _token: '{{csrf_token()}}',
                deleteId: deleteId,
              })
              .then(function (response) {
                ...
                swal(
                  'Deleted!',
                  'Business Setup has been deleted.',
                  'success'
                )
