<script type="text/javascript">
    ...
    var table = $('#datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('mf_our_service.datatable') }}",
        "columns": [{
                "data": "id",
                "defaultContent": ""
            },
            ...
            @if(auth()->user()->can('mf_our_services-edit') || auth()->user()->can('mf_our_services-delete'))
            {
                "data": "id",
                "defaultContent": ""
            },
        ...
        "columnDefs": [{
            ...
            },
            {
                ...
                "render": function(data, type, row, meta) {
                    ...
                    var edit = "{{ route('mf_our_service.edit', [':id']) }}";
                    edit = edit.replace(':id', data);

                    var header_content = "{{ route('mf_our_service.header_content', [':id']) }}";
                    header_content = header_content.replace(':id', data);

                    return `<div class="text-center">
                          
                          @can('mf_our_services-add header content')
        ...
        $('.status').change(function () {
          ...
          axios
            .post('{{route("mf_our_service.status")}}', {
              _token: '{{csrf_token()}}',
              _method: 'patch',
              id: id,
              status: status,
        ...
        $('.delete').click(function () {
          ...
          swal({
            ...
            if (result.value) {
            axios
              .post('{{route("mf_our_service.destroy")}}', {
                _method: 'delete',
                _token: '{{csrf_token()}}',
                deleteId: deleteId,
              })
